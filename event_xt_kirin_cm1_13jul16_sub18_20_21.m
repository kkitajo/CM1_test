...
%for sub = 13:13 % subject number
for sub = [23]
    if sub>=100
        filename=['cm1s',num2str(sub),'.vhdr'];
    else
        filename=['cm1s0',num2str(sub),'.vhdr'];     % Input %
    end;
    ...
    for s=1:length(trigger)
        disp('cm_loop_start');
        disp(cm_num(s));
        f_name=strcat('CM1_S',num2str(cm_num(s)),'.mat');    % for save % Input %
        f_name2=strcat('CM1_S',num2str(cm_num(s)),'_sub',num2str(sub),'_filt','.mat');    % for save % Input %
        ...
        str=['CM1_S',num2str(cm_num(s))];
        str2=['CM1_S',num2str(cm_num(s)),'_sub',num2str(sub),'_filt'];
        
        eval([str2,'=filt_eegdata;'])
        
        save([f_name2],['CM1_S',num2str(cm_num(s)),'_sub',num2str(sub),'_filt'],'-v7.3');      %%% input %%%
        disp(f_name2);
    end
end
